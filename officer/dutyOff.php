...
try {
   ...
   // echo "Successfully connected with  database";
    if (isset($_POST['update'])) {
        ...
        $updateQuery = "UPDATE duty_details SET off_time='$off_time', status='off' WHERE officer_id='$officer_id' ";
        $updateQuery_run = $conn->prepare($updateQuery);
        $updateQuery_exec = $updateQuery_run->execute();

        if ($updateQuery_exec) {
            $message1 = "Duty off Data Successfully updated";
...
<body>
    <main class="pt-0">
        ...
        <p class="btn add-btn fw-bolder btn-rounded btn-lg btn-light"> Update officer duty-off details
        </p>
    </div>
            ...
            <div class="row mx-0 text-ceneter add-btn" style="max-width: 460px;min-width:360px">
                <div class="all mx-1 my-1 ">
                    <div class="row justify-content-between pt-3 px-1 mb-4 all">
                        <div class="col-9">
                            <h4 class="fw-bold">You are updating your duty-off deails </h4>
                        </div>
                        <div class="col-2">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>"
                        ...
                        <div class="mb-3 justify-content-between row all btn-rounded py-1">
                            <label class="form-label pb-n2 text-nowrap col-6 fw-bold">Off Time</label>
                            <input
                                class="all bg-transparent btn py-2 fw-bolder bg-light btn-light all btn-rounded col-6 "
                                type="text" name="off_time" id="time2" onblur="return validate_time2()" required placeholder="Enter the off Time">  
                                <span id="err_time2" class="danger"></span>
                        </div>
                        ...
                        <div class="text-center mt-3 " role="group" aria-label="Basic example">
                            ...
                            <a href="dutyOff.php" class="btn btn-danger btn-rounded fw-bold ">Reset</a>
...
function validate_officerid()
        ...
        function validate_time2()
        {
            ...
            var  time_err = document.getElementById("err_time2");
