...
try {
   ...
   // echo "Successfully connected with  database";
    if (isset($_POST['update'])) {
        ...
        $updateQuery = "UPDATE duty_details SET location='$location' , on_time='$on_time', off_time='$off_time', date='$date' WHERE officer_id='$officer_id' ";
        $updateQuery_run = $conn->prepare($updateQuery);
        $updateQuery_exec = $updateQuery_run->execute();

        if ($updateQuery_exec) {
            $message1 = "Data Successfully updated";
        } else {
                        ...
                        <div class="row mx-0 text-ceneter add-btn" style="max-width: 460px;min-width:360px">
                            <div class="all mx-1 my-1 ">
                                <div class="row justify-content-between pt-3 px-1 mb-4 all">
                                    <div class="col-9">
                                        <h4 class="fw-bold">You are updating your details </h4>
                                    </div>
                                ...
                                <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>"
                                    ...
                                    <div class="text-center mt-3 " role="group" aria-label="Basic example">
                                        ...
                                        <a href="dutyOn.php" class="btn btn-success btn-rounded fw-bold ">Duty-On</a>
                                        <a href="dutyOff.php" class="btn btn-danger btn-rounded fw-bold ">Duty-Off</a>
...
<script>
            ...
            var pattern = /[0-9]{6}/;
                ...
                if(officerid.length==6)
                {
                        ...
                        else
                        {
                            ...
                            err_officerid.style.color="#ffff00";
        ...
        function validate_location()
        {
            ...
            else
            {
                ...
                ocation_err.style.color="#ffff00";
